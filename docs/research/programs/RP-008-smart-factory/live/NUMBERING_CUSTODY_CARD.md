...
Observe, score, and HOLD document-number identity, uniqueness, collision, and custody through the **OT/ERP boundary** lens — observe ERP numbers vs scan/label/MES identities without bridging execute. Do not edit `docs/knowledge/**` or treat knowledge-pack conclusions as live facts.

## Numbering / identity observation points

1. ERP DO/Sample numbers vs warehouse scan labels
2. Same-second collision impact on barcode/print jobs
3. Whether MES work orders key on SO/DO display numbers
4. Reprint/relabel after collision or reopen
5. Cross-system identity when inventory ledger remarks embed DO-no
6. OT clock skew vs timestamp(second) generators
