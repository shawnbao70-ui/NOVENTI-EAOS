"""SQLAlchemy mapping for declared EAOS roles catalog (PHX-G90)."""

from __future__ import annotations

from datetime import datetime
from uuid import UUID

from sqlalchemy import CheckConstraint, DateTime, Index, Integer, String, Uuid, text
from sqlalchemy.orm import Mapped, mapped_column

from kernel.infrastructure.persistence.metadata import Base

STATUSES = "'active','disabled'"


class EaosDeclaredRoleRecord(Base):
    __tablename__ = "eaos_declared_roles"
    __table_args__ = (
        CheckConstraint(f"status IN ({STATUSES})", name="status_valid"),
        CheckConstraint("version > 0", name="version_positive"),
        Index(
            "ix_eaos_declared_roles_name_lower",
            text("lower(name)"),
            unique=True,
        ),
        Index("ix_eaos_declared_roles_status", "status", unique=False),
        {"schema": "kernel"},
    )

    id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True)
    name: Mapped[str] = mapped_column(String(128), nullable=False)
    status: Mapped[str] = mapped_column(String(32), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
