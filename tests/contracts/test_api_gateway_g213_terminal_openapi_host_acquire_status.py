"""PHX-G213 Terminal OpenAPI inventory host-acquire details status deepen contracts."""
...
def test_g213_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0232-terminal-openapi-inventory-host-acquire-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G213_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G213_ARCHITECTURE_GATE.md").is_file()

def test_g213_terminal_ui_wired() -> None:
    ...
    assert "host_acquire_details" in js
    assert "host-acquire details per-code honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g213_inventory_still_semantic_partial() -> None:
...
def test_g213_ledger_tip_manifest_baseline() -> None:
