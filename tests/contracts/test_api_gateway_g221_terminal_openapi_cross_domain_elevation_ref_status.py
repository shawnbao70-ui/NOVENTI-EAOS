"""PHX-G221 Terminal OpenAPI inventory cross-domain elevation $ref status deepen."""
...
def test_g221_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0240-terminal-openapi-inventory-cross-domain-elevation-ref-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G221_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G221_ARCHITECTURE_GATE.md").is_file()

def test_g221_terminal_ui_wired() -> None:
    ...
    assert "cross_domain_elevation_details_ref" in js
    assert "cross-domain elevation details $ref honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g221_inventory_still_semantic_partial() -> None:
...
def test_g221_ledger_tip_manifest_baseline() -> None:
