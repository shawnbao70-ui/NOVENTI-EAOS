"""PHX-G223 Terminal OpenAPI inventory stub detail const status deepen."""
...
def test_g223_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0242-terminal-openapi-inventory-stub-detail-const-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G223_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G223_ARCHITECTURE_GATE.md").is_file()

def test_g223_terminal_ui_wired() -> None:
    ...
    assert "stub_detail_const" in js
    assert "stub detail const honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g223_inventory_still_semantic_partial() -> None:
...
def test_g223_ledger_tip_manifest_baseline() -> None:
