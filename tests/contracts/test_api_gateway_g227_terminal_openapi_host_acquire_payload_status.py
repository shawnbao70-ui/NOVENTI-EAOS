"""PHX-G227 Terminal OpenAPI inventory HostAcquirePayload status deepen."""
...
def test_g227_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0246-terminal-openapi-inventory-host-acquire-payload-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G227_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G227_ARCHITECTURE_GATE.md").is_file()

def test_g227_terminal_ui_wired() -> None:
    ...
    assert "host_acquire_payload_named" in js
    assert "HostAcquirePayload named honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g227_inventory_still_semantic_partial() -> None:
...
def test_g227_ledger_tip_manifest_baseline() -> None:
