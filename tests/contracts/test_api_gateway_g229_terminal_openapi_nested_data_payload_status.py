"""PHX-G229 Terminal OpenAPI inventory nested data payload status deepen."""
...
def test_g229_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0248-terminal-openapi-inventory-nested-data-payload-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G229_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G229_ARCHITECTURE_GATE.md").is_file()

def test_g229_terminal_ui_wired() -> None:
    ...
    assert "nested_data_payload_named" in js
    assert "nested data payload named honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g229_inventory_still_semantic_partial() -> None:
...
def test_g229_ledger_tip_manifest_baseline() -> None:
