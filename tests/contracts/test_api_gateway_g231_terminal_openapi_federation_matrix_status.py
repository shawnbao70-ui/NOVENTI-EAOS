"""PHX-G231 Terminal OpenAPI inventory federation matrix status deepen."""
...
def test_g231_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0250-terminal-openapi-inventory-federation-matrix-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G231_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G231_ARCHITECTURE_GATE.md").is_file()

def test_g231_terminal_ui_wired() -> None:
    ...
    assert "federation_matrix_payload_named" in js
    assert "federation matrix payload named honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g231_inventory_still_semantic_partial() -> None:
...
def test_g231_ledger_tip_manifest_baseline() -> None:
