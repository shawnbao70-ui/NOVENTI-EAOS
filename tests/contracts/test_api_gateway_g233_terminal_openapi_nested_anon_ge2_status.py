"""PHX-G233 Terminal OpenAPI inventory nested-anon≥2 status deepen."""
...
def test_g233_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0252-terminal-openapi-inventory-nested-anon-ge2-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G233_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G233_ARCHITECTURE_GATE.md").is_file()

def test_g233_terminal_ui_wired() -> None:
    ...
    assert "nested_anon_ge2_payload_named" in js
    assert "nested-anon≥2 payload named honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g233_inventory_still_semantic_partial() -> None:
...
def test_g233_ledger_tip_manifest_baseline() -> None:
