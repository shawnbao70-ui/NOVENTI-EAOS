"""PHX-G235 Terminal OpenAPI inventory CountMeta status deepen."""
...
def test_g235_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0254-terminal-openapi-inventory-count-meta-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G235_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G235_ARCHITECTURE_GATE.md").is_file()

def test_g235_terminal_ui_wired() -> None:
    ...
    assert "count_meta_and_oidc_providers_payload_named" in js
    assert "CountMeta/OidcProvidersPayload named honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g235_inventory_still_semantic_partial() -> None:
...
def test_g235_ledger_tip_manifest_baseline() -> None:
