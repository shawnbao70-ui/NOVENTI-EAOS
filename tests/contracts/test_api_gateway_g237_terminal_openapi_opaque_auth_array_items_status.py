"""PHX-G237 Terminal OpenAPI inventory opaque auth array-item status deepen."""
...
def test_g237_docs_present() -> None:
    assert (
        ROOT
        / "docs"
        / "decisions"
        / "ADR-0256-terminal-openapi-inventory-opaque-auth-array-items-status-deepen.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G237_ACCEPTANCE.md").is_file()
    assert (ROOT / "docs" / "project" / "PHX-G237_ARCHITECTURE_GATE.md").is_file()

def test_g237_terminal_ui_wired() -> None:
    ...
    assert "opaque_auth_array_items_named" in js
    assert "opaque auth array-item named honest" in js
    assert 'id="btnAdminOpenapiInventoryStatus"' in html
    assert "G28" in html
    assert "loadOpenapiInventoryProductPosture({ quiet: true })" in js

def test_g237_inventory_still_semantic_partial() -> None:
...
def test_g237_ledger_tip_manifest_baseline() -> None:
