"""PHX-G282 Foundation contract softener wave5 honesty."""
...
def test_g282_docs_present() -> None:
    assert (
        ROOT / "docs" / "decisions" / "ADR-0301-foundation-contract-softener-wave5.md"
    ).is_file()
    assert (ROOT / "docs" / "project" / "PHX-G282_ACCEPTANCE.md").is_file()

def test_g282_ops_tip_parity() -> None:
    ...
    assert str(ops["info"]["version"]).startswith("1.0.")
    assert "g282" in " ".join(posture["fail_closed_reasons"]).casefold()
    meta = TestClient(app).get("/v1/adapters").json()["meta"]["openapi_inventory_product"]
    assert meta["milestone"].startswith("PHX-G")

def test_g282_baseline() -> None:
