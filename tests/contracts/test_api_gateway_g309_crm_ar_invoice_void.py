"""PHX-G309 CRM AR Invoice Void HTTP contracts."""
...
def _headers() -> dict[str, str]:
    return {
        ...
        "X-Correlation-Id": "corr-g309-http",
    }
...
def _issued_invoice(client: TestClient) -> dict:
    customer = client.post(
        "/v1/crm/customers",
        headers=_headers(),
        json={"code": f"C16-{uuid4().hex[:8]}", "display_name": "C16 API Customer"},
    ).json()["data"]
    opportunity = client.post(
        "/v1/crm/opportunities",
        headers=_headers(),
        json={"customer_id": customer["id"], "title": "C16 Opportunity"},
    ).json()["data"]
    requirement = client.post(
        "/v1/crm/requirements",
        headers=_headers(),
        json={"opportunity_id": opportunity["id"], "title": "C16 Requirement"},
    ).json()["data"]
    quote = client.post(
        "/v1/crm/quotes",
        headers=_headers(),
    ...
    assert (
        client.post(
            ...
            headers=_headers(),
            json={"description": "C16 line", "quantity": "2", "unit_price": "10"},
        ).status_code
        == 201
    )
    assert (
...
def test_g309_void_round_trip() -> None:
    client = _client()
    invoice = _issued_invoice(client)
    response = client.post(
        f"/v1/crm/ar-invoices/{invoice['id']}/void",
        headers=_headers(),
        json={
    ...
    voided = response.json()["data"]
    assert voided["status"] == "voided"
    assert voided["voided_at"] is not None
    assert voided["void_reason"] == "Operator retract"
    fetched = client.get(
...
def test_g309_rejects_context_override() -> None:
    client = _client()
    invoice = _issued_invoice(client)
    response = client.post(
        f"/v1/crm/ar-invoices/{invoice['id']}/void",
...
def test_g309_openapi_forbids_finance_reverse_surfaces() -> None:
    spec = _client().get("/openapi.json").json()
    assert "/v1/crm/ar-invoices/{invoice_id}/void" in spec["paths"]
    paths = " ".join(
        path
    ...
    assert (
        spec["components"]["schemas"]["VoidARInvoiceRequest"][
