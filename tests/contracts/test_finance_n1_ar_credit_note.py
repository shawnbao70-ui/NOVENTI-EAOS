"""PHX-G312 Finance AR Credit Note N1 contracts."""
...
def _ctx() -> ExecutionContext:
    return ExecutionContext(
        ...
        tenant_id=uuid4(),
        correlation_id=f"corr-g312-{uuid4()}",
        request_time=ExecutionContext.utc_now(),
    )


def _services(ctx: ExecutionContext, *, grant_credit: bool = True):
    ...
    credit_actions = (
        {"create", "read", "issue"} if grant_credit else {"read"}
    )
    assert permission.grant(
        ctx,
        principal_subject_id=ctx.subject_id,
...
def _issued_invoice(crm: CRMService, ctx: ExecutionContext):
    customer = crm.create_customer(
        ctx, code=f"N1-{uuid4().hex[:8]}", display_name="N1 Customer"
    ).data
    assert customer is not None
    opportunity = crm.create_opportunity(
        ctx, customer_id=customer.id, title="N1 Opportunity"
    ).data
    assert opportunity is not None
    requirement = crm.create_requirement(
        ctx, opportunity_id=opportunity.id, title="N1 Requirement"
    ).data
...
def test_n1_create_and_issue_against_issued_invoice() -> None:
    ctx = _ctx()
    ...
    retry = finance.create_credit_note(
        ctx,
        invoice_id=invoice.id,
        amount=Decimal("5.00"),
        idempotency_key=create_key,
    ...
    issue_key = uuid4()
    issued = finance.issue_credit_note(
        ctx,
    ...
    replay = finance.issue_credit_note(
        ctx,
        credit_note_id=created.data.id,
        idempotency_key=issue_key,
        human_confirm=True,
    ...
    events = [
        ...
        if event.action.startswith("Finance.ARCreditNote.")
    ]
    assert all(event.details == {} for event in events)


def test_n1_allows_voided_rejects_draft_and_over_amount() -> None:
    ctx = _ctx()
    ...
    against_voided = finance.create_credit_note(
        ctx,
        invoice_id=invoice.id,
        amount=Decimal("1.00"),
        idempotency_key=uuid4(),
    )
    assert against_voided.ok and against_voided.data is not None

    draft = _issued_invoice(crm, ctx)
    # build another draft invoice
    customer = crm.create_customer(
        ctx, code=f"N1D-{uuid4().hex[:8]}", display_name="Draft"
    ).data
    assert customer is not None
    opportunity = crm.create_opportunity(
    ...
    assert (
        finance.create_credit_note(
            ctx,
            invoice_id=draft.id,
            amount=draft.total_amount + Decimal("0.01"),
            idempotency_key=uuid4(),
        ).error_code
...
def test_n1_default_deny_is_audited() -> None:
    ctx = _ctx()
    crm, finance, audit = _services(ctx, grant_credit=False)
    invoice = _issued_invoice(crm, ctx)
    denied = finance.create_credit_note(
        ctx,
        invoice_id=invoice.id,
        amount=Decimal("1.00"),
        idempotency_key=uuid4(),
    ...
    events = [
        event
        for event in audit.list_events()
        if event.action.startswith("Finance.ARCreditNote.Create")
    ]
