"""PHX-G316 Finance Tax Invoice Tax1 contracts."""
...
def _ctx() -> ExecutionContext:
    return ExecutionContext(
        ...
        tenant_id=uuid4(),
        correlation_id=f"corr-g316-{uuid4()}",
        request_time=ExecutionContext.utc_now(),
    )


def _services(ctx: ExecutionContext, *, grant_tax: bool = True):
    ...
    tax_actions = (
        {"create", "read", "issue", "void"} if grant_tax else {"read"}
    )
    assert permission.grant(
        ctx,
        principal_subject_id=ctx.subject_id,
        resource_type=TAX_INVOICE_RESOURCE,
        actions=tax_actions,
        scope_level=ScopeLevel.TENANT,
    ).ok
...
def _issued_invoice(crm: CRMService, ctx: ExecutionContext):
    customer = crm.create_customer(
        ctx, code=f"TAX1-{uuid4().hex[:8]}", display_name="Tax1 Customer"
    ).data
    assert customer is not None
    opportunity = crm.create_opportunity(
        ctx, customer_id=customer.id, title="Tax1 Opportunity"
    ).data
    assert opportunity is not None
    requirement = crm.create_requirement(
        ctx, opportunity_id=opportunity.id, title="Tax1 Requirement"
    ).data
...
def test_tax1_create_issue_void_against_issued_invoice() -> None:
    ctx = _ctx()
    crm, finance, audit = _services(ctx)
    invoice = _issued_invoice(crm, ctx)
    create_key = uuid4()
    created = finance.create_tax_invoice(
        ctx,
        invoice_id=invoice.id,
    ...
    assert created.data.customer_id == invoice.customer_id
    assert created.data.currency == invoice.currency
    retry = finance.create_tax_invoice(
        ctx,
        invoice_id=invoice.id,
        amount=Decimal("20.00"),
        idempotency_key=create_key,
    ...
    issue_key = uuid4()
    issued = finance.issue_tax_invoice(
        ctx,
        tax_invoice_id=created.data.id,
        idempotency_key=issue_key,
        human_confirm=True,
    )
    assert issued.ok and issued.data is not None
    assert issued.data.status.value == "issued"
    replay = finance.issue_tax_invoice(
        ctx,
        tax_invoice_id=created.data.id,
        idempotency_key=issue_key,
        human_confirm=True,
    ...
    assert replay.data.id == issued.data.id
    void_key = uuid4()
    voided = finance.void_tax_invoice(
        ctx,
        tax_invoice_id=created.data.id,
        idempotency_key=void_key,
        human_confirm=True,
        reason="cancel filing intent",
    )
    assert voided.ok and voided.data is not None
    assert voided.data.status.value == "voided"
    assert voided.data.void_reason == "cancel filing intent"
    void_replay = finance.void_tax_invoice(
        ctx,
        tax_invoice_id=created.data.id,
        idempotency_key=void_key,
        human_confirm=True,
    ...
    assert void_replay.ok and void_replay.data is not None
    assert void_replay.data.status.value == "voided"
    assert (
        finance.issue_tax_invoice(
            ctx,
            tax_invoice_id=created.data.id,
            idempotency_key=uuid4(),
            human_confirm=True,
    ...
    events = [
        ...
        if event.action.startswith("Finance.TaxInvoice.")
    ]
    assert all(event.details == {} for event in events)


def test_tax1_rejects_draft_voided_invoice_and_over_amount() -> None:
    ctx = _ctx()
    crm, finance, _ = _services(ctx)
    issued = _issued_invoice(crm, ctx)
    assert (
        finance.create_tax_invoice(
            ctx,
            invoice_id=issued.id,
            amount=issued.total_amount + Decimal("0.01"),
            idempotency_key=uuid4(),
        ).error_code
    ...
    customer = crm.create_customer(
        ctx, code=f"TAX1D-{uuid4().hex[:8]}", display_name="Draft"
    ).data
    assert customer is not None
    opportunity = crm.create_opportunity(
        ctx, customer_id=customer.id, title="Draft Opp"
    ).data
    assert opportunity is not None
    requirement = crm.create_requirement(
        ctx, opportunity_id=opportunity.id, title="Draft Req"
    ).data
    assert requirement is not None
    quote = crm.create_quote(ctx, requirement_id=requirement.id).data
    assert quote is not None
...
def test_tax1_default_deny_is_audited() -> None:
    ctx = _ctx()
    crm, finance, audit = _services(ctx, grant_tax=False)
    invoice = _issued_invoice(crm, ctx)
    denied = finance.create_tax_invoice(
        ctx,
        invoice_id=invoice.id,
        amount=Decimal("1.00"),
        idempotency_key=uuid4(),
    ...
    events = [
        event
        for event in audit.list_events()
        if event.action.startswith("Finance.TaxInvoice.Create")
    ]
