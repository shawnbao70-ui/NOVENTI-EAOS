"""NRI RP-003 Architecture Review Candidate Package contracts."""
...
ARC = (
    ...
    / "research"
    / "programs"
    / "RP-003-capability-first"
    / "ARCHITECTURE_REVIEW_CANDIDATE-RP-003.md"
)
README = ROOT / "docs" / "research" / "programs" / "RP-003-capability-first" / "README.md"
DELIV = (
    ROOT
    / "docs"
    / "research"
    / "programs"
    / "RP-003-capability-first"
    / "DELIVERABLES-RP-003.md"
)
EVID = ROOT / "docs" / "research" / "programs" / "RP-003-capability-first" / "EVIDENCE_PACK.md"
...
def test_rp003_arc_candidate_exists_and_awaits_board() -> None:
    assert ARC.is_file()
    text = ARC.read_text(encoding="utf-8")
    assert "NRI-ARC-RP-003" in text
    assert (
    ...
    assert "Cap≠Org" in text or "Cap != Org" in text or "Cap ≠ Org" in text
    assert "Capability" in text and ("Permission" in text or "grant" in text.casefold())
    assert "auto_grant_minted" in text
    assert "CFM" in text or "Capability First" in text
    assert "CG-01" in text or "CG-02" in text or "CG-01…02" in text
...
def test_rp003_arc_linked_in_program_and_registry() -> None:
    readme = README.read_text(encoding="utf-8")
    assert "ARCHITECTURE_REVIEW_CANDIDATE" in readme
    assert "NRI-ARC-RP-003" in readme or "ARCHITECTURE_REVIEW_CANDIDATE" in readme
    assert "Accepted" in readme
    deliv = DELIV.read_text(encoding="utf-8")
    evid = EVID.read_text(encoding="utf-8")
    assert "ARCHITECTURE_REVIEW_CANDIDATE" in deliv or "NRI-ARC-RP-003" in deliv or "Architecture Review Candidate" in deliv
    assert "ARCHITECTURE_REVIEW_CANDIDATE" in evid or "NRI-ARC-RP-003" in evid or "Architecture Review Candidate" in evid
    index = INDEX.read_text(encoding="utf-8")
    library = LIBRARY.read_text(encoding="utf-8")
    assert "NRI-ARC-RP-003" in index
    assert "NRI-ARC-RP-003" in library
    tip = TIP.read_text(encoding="utf-8")
    assert "RP-003" in tip
    assert "Architecture Review" in tip or "AR" in tip
    assert "NRI-ARC-RP-003" in tip or "ARCHITECTURE_REVIEW_CANDIDATE" in tip or "Candidate" in tip
    ledger = LEDGER.read_text(encoding="utf-8")
    assert "DAL-U018" in ledger
    assert "RP-003" in ledger or "NRI-ARC-RP-003" in ledger
    # Preserve concurrent U016/U017/U019 if present (other Wave/G2 deepenings may land alongside)
    if "DAL-U016" in ledger:
        assert "DAL-U016" in ledger
    if "DAL-U017" in ledger:
        assert "DAL-U017" in ledger
    if "DAL-U019" in ledger:
