"""Destructive PostgreSQL acceptance for PHX-G310 Finance AR Receipt F1."""
...
from noventi.finance.service import AR_RECEIPT_RESOURCE
...
def _platform_context() -> ExecutionContext:
    return ExecutionContext(
        ...
        tenant_id=None,
        platform_scope=True,
        correlation_id=f"fin-platform-{uuid4()}",
...
def _tenant_context(
    ...
    return ExecutionContext(
        ...
        tenant_id=tenant_id,
        correlation_id=f"fin-tenant-{uuid4()}",
        request_time=ExecutionContext.utc_now(),
    )
...
def test_g310_migration_creates_finance_receipt_schema(
    postgres_engine: Engine,
) -> None:
    inspector = inspect(postgres_engine)
    assert set(inspector.get_table_names(schema="finance")) >= {"ar_receipts"}
...
def test_g310_apply_receipt_to_issued_invoice_persists(
    postgres_engine: Engine,
) -> None:
    factory = create_session_factory()
    _, ctx = _tenant_actor(factory, "Finance F1 Tenant")
    ...
    customer = crm.create_customer(
        ctx, code=f"F1-{uuid4().hex[:8]}", display_name="F1 PG Customer"
    ).data
    assert customer is not None
    opportunity = crm.create_opportunity(
        ctx, customer_id=customer.id, title="F1 Opp"
    ).data
    assert opportunity is not None
    requirement = crm.create_requirement(
        ctx, opportunity_id=opportunity.id, title="F1 Req"
    ).data
    ...
    with postgres_engine.connect() as connection:
        row = connection.execute(
            text(
                "SELECT status, ar_invoice_id, amount, allocated_amount "
                "FROM finance.ar_receipts WHERE id = :id"
            ),
            {"id": created.data.id},
        ).one()
        ...
        assert row.allocated_amount == Decimal("20.00")
        assert (
            connection.scalar(text("SELECT version_num FROM alembic_version"))
            == EXPECTED_TIP
        )
