"""Destructive PostgreSQL acceptance for PHX-G311 Inventory DO Ship I1."""
...
from noventi.inventory.service import DELIVERY_SHIP_RESOURCE, STOCK_RESOURCE
...
def _platform_context() -> ExecutionContext:
    return ExecutionContext(
        ...
        tenant_id=None,
        platform_scope=True,
        correlation_id=f"inv-platform-{uuid4()}",
...
def _tenant_context(
    ...
    return ExecutionContext(
        ...
        tenant_id=tenant_id,
        correlation_id=f"inv-tenant-{uuid4()}",
        request_time=ExecutionContext.utc_now(),
    )
...
def _tenant_actor(factory, name: str) -> tuple[UUID, ExecutionContext]:
    ...
    actor = identity.register_subject(
        ...
        display_name=f"{name} Inventory Operator",
    )
...
def test_g311_migration_creates_inventory_schema(
    postgres_engine: Engine,
) -> None:
    inspector = inspect(postgres_engine)
    tables = set(inspector.get_table_names(schema="inventory"))
    assert {"delivery_ship_postings", "ledger_entries", "stock_balances"} <= tables
...
def test_g311_ship_persists_ledger_and_do_status(
    postgres_engine: Engine,
) -> None:
    factory = create_session_factory()
    _, ctx = _tenant_actor(factory, "Inventory I1 Tenant")
    ...
    customer = crm.create_customer(
        ctx, code=f"I1-{uuid4().hex[:8]}", display_name="I1 PG"
    ).data
    assert customer is not None
    opportunity = crm.create_opportunity(
        ctx, customer_id=customer.id, title="I1 Opp"
    ).data
    assert opportunity is not None
    requirement = crm.create_requirement(
        ctx, opportunity_id=opportunity.id, title="I1 Req"
    ).data
    ...
    with postgres_engine.connect() as connection:
        ...
        on_hand = connection.scalar(
            text(
                "SELECT on_hand FROM inventory.stock_balances "
                "WHERE sales_order_line_id = :id"
            ),
            {"id": lines[0].id},
        )
        assert on_hand == Decimal("0.0000")
        entry_type = connection.scalar(
            text(
                "SELECT entry_type FROM inventory.ledger_entries "
                "WHERE delivery_order_id = :id LIMIT 1"
            ),
            {"id": released.id},
        )
        assert entry_type == "do_ship"
        assert (
            connection.scalar(text("SELECT version_num FROM alembic_version"))
            == EXPECTED_TIP
        )
